...
public class Collision2 : MonoBehaviour
{
    ...
    {
        instance = this;
        rigid = instance.enemy.GetComponent<Rigidbody2D>();
        animator = instance.enemy.GetComponent<Animator>();
        spriter = instance.enemy.GetComponent<SpriteRenderer>();
        coll = instance.enemy.GetComponent<BoxCollider2D>();
        horiVec = Vector2.right;
        vertVec = Vector2.up;
        horiAlpVec = horiVec * 0.02f;
        vertAlpVec = vertVec * 0.02f;
    }
    ...
    public void CollisionCheck()
    {
        ...
        for (int i = 0; i < layer[mapIndex].coll_count; i++)
        {
                ...
                || layer[mapIndex].layerPosMin[i].x >= playerPosMin.x && layer[mapIndex].layerPosMax[i].x <= playerPosMax.x)
            {
                ...
                {
                    Collision2.instance.enemy.nextVec.y = Mathf.Clamp(Collision2.instance.enemy.nextVec.y,0,1);
                    hitdown = true;
                    jumpCount = 2;
                    spaceTrigger = true;
                    jumpTrigger = true;
                ...
                {
                    Collision2.instance.enemy.nextVec.y = Mathf.Clamp(Collision2.instance.enemy.nextVec.y,-1,0);
                    gravity = "falling";
                    hitup = true;
                }
            }
                ...
                || layer[mapIndex].layerPosMin[i].y >= playerPosMin.y && layer[mapIndex].layerPosMax[i].y <= playerPosMax.y)
            {
                if (layer[mapIndex].layerPosMin[i].x < playerPosMin.x - horiAlpVec.x && playerPosMin.x - horiAlpVec.x < layer[mapIndex].layerPosMax[i].x
                    && horiVec.x < -0.1)
                {
                    Collision2.instance.enemy.nextVec.x = Mathf.Clamp(Collision2.instance.enemy.nextVec.x,0,1);
                    wallcount++;
                    hitleft = true;
                ...
                && horiVec.x > 0.1)
                {
                    Collision2.instance.enemy.nextVec.x = Mathf.Clamp(Collision2.instance.enemy.nextVec.x,-1,0);
                    wallcount++;
                    hitright = true;
